package powercrystals.minefactoryreloaded.plugin.jei.lavafabricator;
...
public class LavaFabricatorRecipeCategory extends MachineRecipeCategory<LavaFabricatorRecipeWrapper> {

    private final IDrawable background;
    private final IDrawable tankOverlay;
    private final IDrawable energyOverlay;

    private final int lavaPerOperation;
    private final int energyPerOperation;
    private final int tankSize;

    private static final int ENERGY_X = 140, ENERGY_Y = 15;

    public LavaFabricatorRecipeCategory(IGuiHelper guiHelper) {

        super(Machine.LavaFabricator);
    ...
    @Override
    public void setRecipe(IRecipeLayout recipeLayout, @Nonnull LavaFabricatorRecipeWrapper recipeWrapper, IIngredients ingredients) {
    ...
    public void registerRecipesHandling(@Nonnull IModRegistry registry) {
        ...
        List<LavaFabricatorRecipeWrapper> recipes = Collections.singletonList(
                new LavaFabricatorRecipeWrapper(new FluidStack(FluidRegistry.LAVA, lavaPerOperation))
        );
